...
mitchells_data <- utils::read.csv("C:/Users/Usuario/Desktop/WHEP_ALE/DATA/EXTRACTED/mitchells_data.csv")
...
# data_manual <- data_manual |> rename(year = Year, item = Item, unit = Unit, country = Area)
# # Merge the datasets, ensuring NA values in `data_manual` are replaced with `mitchells_data` values
# merged_data <- full_join(mitchells_data, data_manual, by = c("year", "country", "item", "unit")) |>
#   dplyr::mutate(value = ifelse(is.na(value.y), value.x, value.y)) |>
#   dplyr::select(year, country, item, unit, value, source_file, sheet, everything()) |>
#   dplyr::select(-value.x, -value.y) |>
# # erase NA rows
#   dplyr::filter(!is.na(value))
#
# # Check missing countries after merging
# missing_countries <- anti_join(mitchells_data, data_manual, by = "country") |> distinct(country)
#
# merged_data <- merged_data |> mutate(year = as.integer(year), Item_Code = as.numeric(Item_Code))
#
# ##Adding FAOSTAT dataset
# primary_all <- utils::read.csv(paste0(L_files_path,"Global/output/Primary_all.csv")) |>
#   dplyr::mutate(Source = "FAO", item_code = as.numeric(item_code))
#
# primary_all <- primary_all |> rename(year = Year, country = area, Area_Code = area_code, value = Value)
#
# # merging datasets
#
...
#   left_join(item_code_lookup, by = c("item" = "item")) |>
#   mutate(
#     Item_Code = case_when(
...
# ## MERGING!
#
# colnames(regions_full)
#
# # NOW, MERGING WITH REGIONS_FULL DATASET
#
...
#
# final_data_full <- merged_data_updated |>
#   dplyr::left_join(country_to_area_code, by = "country") |>
#   dplyr::mutate(Area_Code = ifelse(is.na(Area_Code), area_code, Area_Code)) |>
#   dplyr::select(-area_code) |>
#   dplyr::rename(area_code = Area_Code) |>
#   dplyr::left_join(
#     regions_full |>
#       dplyr::rename(area_code = code) |>
#       dplyr::select(area_code, iso3c),
...
mitchells_data_filtered <- mitchells_data |>
  ...
  filter(!is.na(year)) # Remove any rows with NA after conversion
...
item_code_lookup <- primary_all |>
  ...
  distinct(item, .keep_all = TRUE) # Ensure no duplicates
...
merged_data <- full_join(merged_data, primary_all, by = c("year", "country", "item", "unit")) |>
  mutate(
    value = coalesce(value.y, value.x), # Merge value columns
    Source = coalesce(Source.x, Source.y) # Merge Source columns
  ) |>
  dplyr::select(year, country, item, unit, value, Source, source_file, sheet, -value.x, -value.y, -Source.x, -Source.y)
...
final_data_full <- final_data |>
  ...
  left_join(
    regions_full |>
      dplyr::select(iso3c, name, region_UN) |>
      rename(country_name = name), # Use distinct name to avoid confusion
    by = "iso3c"
  ) |>
...
write_delim(before_1960, "before_1960.csv", delim = "\t", quote = "none")
...
final_data_full |>
  dplyr::filter(item == "sugar" | year <= 1961 | unit == "ha") |>
...
before_1960 |>
  dplyr::filter(item == "Maize (corn)" | unit == "MT") |>
  dplyr::group_by(year, region_UN, unit, Source) |>
  dplyr::summarize(
    value = sum(value, na.rm = TRUE) / 1000000,
    n = n(),
